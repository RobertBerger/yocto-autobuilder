...
then
    ...
    echo "+ $0 reliableembeddedsystems/yocto-autobuilder br0"
...
if [ ! -d /opt/labs-volume ]; then
  sudo mkdir /opt/labs-volume
  sudo chmod 777 /opt/labs-volume
fi

echo "+ ID=\$(docker run -v /opt:/srv -v /tftpboot:/tftpboot -v /opt/labs-volume:/home/genius/labs -v /opt/yocto-autobuilder-volume:/tmp/yocto-autobuilder -t -i -d -p 22 -p 8010 -p 8000 -p 8200 --privileged ${IMAGE_NAME} /sbin/my_init -- bash -l)"
ID=$(docker run -v /opt:/srv -v /tftpboot:/tftpboot -v /opt/labs-volume:/home/genius/labs -v /opt/yocto-autobuilder-volume:/tmp/yocto-autobuilder -t -i -d -p 22 -p 8010 -p 8000 -p 8200 --privileged ${IMAGE_NAME} /sbin/my_init -- bash -l)
